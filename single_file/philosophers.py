from asynclib import EventLoop, Semaphore, Event, coroutine

events = {'Plato': Event(), 'Socrates': Event(), 'Euclid': Event()}
...
forks = [Semaphore(loop=loop) for i in range(3)]
...
loop.run_until_complete(all_gone())
